...
import { STREAM_QUALITIES } from "./quality";
...
export function startBroadcast(options) {
    ...
    const scaleResolutionDownBy = sourceWidth && sourceHeight
        ? Math.max(1, Math.round(Math.min(sourceWidth / preset.width, sourceHeight / preset.height) * 100) / 100)
        : 1;
    const applyQuality = (pc) => {
        ...
        if (scaleResolutionDownBy > 1) {
            encoding.scaleResolutionDownBy = Math.max(encoding.scaleResolutionDownBy ?? 1, scaleResolutionDownBy);
        }
        encoding.maxFramerate = Math.min(encoding.maxFramerate ?? preset.maxFramerate, preset.maxFramerate);
